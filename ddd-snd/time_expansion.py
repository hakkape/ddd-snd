...
class DiscretizedGraph(rx.PyDiGraph):
    ...
    def _add_travel_arcs(self):
        for arc in self.g_flat.edge_indices():
            ...
            for start_node in potential_start_nodes:
                ...
                if not self.relaxed:
                    if (
                        self.g_ex[potential_end_nodes[end_node_index]].time
                        != start_time + travel_time
                    ):
                        offset = 1
                ...
                arc_ex = self.g_ex.add_edge(start_node, end_node, arc_data)
    ...
    def _shorten_travel_arcs_unrelaxed(self, new_node: int, next_node: int, time: int):
        ...
        ingoing_arcs = self.g_ex.in_edges(
            next_node
        ...
        for i, j, data in ingoing_arcs:
            # skip holding arcs
            if self.g_ex[i].flat_id == self.g_ex[j].flat_id:
                continue
            arrival_time = self.g_ex[i].time + data.travel_time
            after_time = self.g_ex[next_node].time
            if arrival_time < after_time and arrival_time >= time:
                # remove old edge
                edge_to_remove = get_edge_index(self.g_ex, i, j)
                self.g_ex.remove_edge(edge_to_remove)  # from graph
                self.flat_to_expanded_arcs[j].remove(edge_to_remove)  # from mapping

                # add new edge
                new_edge = self.g_ex.add_edge(i, new_node, data)  # to graph
    ...
    def _refine_holding_arc(self, new_node: int, previous_node: int, next_node: int):
        ...
        holding_arc = get_edge_index(self.g_ex, previous_node, next_node)
        self.g_ex.remove_edge(holding_arc)

        # add new holding arcs
        self.g_ex.add_edge(previous_node, new_node, ArcData(0, 0, 0, None))
        self.g_ex.add_edge(new_node, next_node, ArcData(0, 0, 0, None))

    def _add_travel_arcs_new_node(self, new_node: int):
        # get arcs outgoing from the corresponding flat node
        flat_node = self.g_ex[new_node].flat_id
        outgoing_arcs = self.g_flat.out_edges(flat_node)
        for i, j, data in outgoing_arcs:
            arrival_time = self.g_ex[new_node].time + data.travel_time
            ...
            if self.relaxed:
                ...
                if self.g_ex[j_ex].time != arrival_time:
                    j_ex = self.flat_to_expanded_nodes[j][k_j - 1]
            # add new travel arc
            self.g_ex.add_edge(new_node, j_ex, data)  # to graph
    ...
    def _lengthen_travel_arcs_relaxed(
        ...
        ingoing_arcs = self.g_ex.in_edges(previous_node)
        for i, j, data in ingoing_arcs:
            # skip holding arcs
            if self.g_ex[i].flat_id == self.g_ex[j].flat_id:
                continue
            ...
            if arrival_time >= time:
                # remove old edge
                edge_to_remove = get_edge_index(self.g_ex, i, previous_node)
                self.g_ex.remove_edge(edge_to_remove)  # from graph
